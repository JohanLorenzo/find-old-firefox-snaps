...
def get_promote_graph_ids():
    all_releases = requests.get('https://shipit-api.mozilla-releng.net/releases?status=shipped').json()
    firefox_only = (
        release
        for release in all_releases
        if release.get('product') == 'firefox'
    )
    with_phases = (
        release
        for release in firefox_only
        if release.get('phases')
    )

    for release in with_phases:
        ...
        promote_graph_ids = [
            ...
            if phase['name'] in ('promote_firefox', 'promote_firefox_rc')
        ]

        if len(promote_graph_ids) != 1:
            raise Exception('version "{}" build {} does not have a singular promote graph: {}'.format(version, build_number, release))
...
for promote_graph_id, version, build_number in get_promote_graph_ids():
